...
import type { AgentCommand, AgentToControl, ControlToAgent } from "./ipc.js";
...
export class Agent {
  ...
  private readonly emit: Emit = (event) => this.send({ type: "event", event });

  constructor(
    private readonly config: AgentConfig,
    private readonly send: (msg: AgentToControl) => void,
  ) {}
  ...
  async start(): Promise<void> {
    ...
    this.send({ type: "ready", agentId: this.config.agentId });
  ...
  async onMessage(msg: ControlToAgent): Promise<void> {
    ...
    if (msg.type === "cmd") {
      try {
        ...
        this.send({ type: "cmd-result", id: msg.id, ok: true, data });
      } catch (err) {
        this.send({ type: "cmd-result", id: msg.id, ok: false, error: (err as Error).message });
      }
    }
  }
